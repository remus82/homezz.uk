...
class CWebLanguage extends BaseModel {
  ...
  public function doModel() {
    ...
    } else if(preg_match('/\/[a-z]{2}-[A-Z]{2}\//', $redirect_url) && $changed) {
      $redirect_url = preg_replace('/\/[a-z]{2}-[A-Z]{2}\//', '/' . str_replace('_', '-', $locale) . '/', $redirect_url);

    // URL contains language in format .../en_US/...
    } else if(preg_match('/\/[a-z]{2}_[A-Z]{2}\//', $redirect_url) && $changed) {
      $redirect_url = preg_replace('/\/[a-z]{2}_[A-Z]{2}\//', '/' . $locale . '/', $redirect_url);
    }
